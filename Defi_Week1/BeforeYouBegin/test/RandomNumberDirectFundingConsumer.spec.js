...
!developmentChains.includes(network.name)
  ...
  : describe('Random Number Direct Funding Consumer Unit Tests', async () => {
      ...
      const nftMintingGas = BigNumber.from(183_333);
      ...
      const calculatePriceNFT = (
        ...
        _wrapperPremium = wrapperPremiumPercentage,
        _flatFee = flatFee,
        _nftMintingGas = nftMintingGas
      ) => {
        const totalGas = BigNumber.from(0).add(gasLimit).add(_wrapperGasOverhead).add(_coordinatorGasOverhead).add(_nftMintingGas);
        const baseFee = BigNumber.from('1000000000000000000').mul(_gasPriceWei).mul(totalGas).div(_weiPerUnitLink);
        const withPremium = baseFee.mul(BigNumber.from(100).add(_wrapperPremium)).div(100);
        return withPremium.add(_flatFee);
      };
      ...
      const deployRandomNumberConsumerFixture = async () => {
        ...
        const consumerFactory = await ethers.getContractFactory('NFT_VRF_HH', consumerOwner);
      ...
      describe('#requestRandomWords', async () => {
        describe('success', async () => {
          it('Should successfully request a random number', async () => {
            const { consumer, wrapper, coordinator, link, owner, consumerOwner } = await loadFixture(deployRandomNumberConsumerFixture);
            ...
            await expect(
              consumer.connect(consumerOwner).requestRandomWords(gasLimit, 3, numWords, {
                gasPrice: oneHundredGwei,
              })
            )
          ...
          it('Should successfully request a random number and get a result', async () => {
            ...
            const { balance: initialSubscriptionbalance } = await coordinator.getSubscription(subscriptionId);
            const gasLimit = 100_000;
            const price = calculatePrice(gasLimit);
            const numWords = 1;

            await consumer.connect(consumerOwner).requestRandomWords(gasLimit, 3, numWords, {
              gasPrice: oneHundredGwei,
            });
            ...
            const { paid, fulfilled, randomWords } = await consumer.getRequestStatus(requestId);
            expect(paid).to.equal(price);
            ...
            const { balance: finalSubscriptionbalance } = await coordinator.getSubscription(subscriptionId);

            console.log('price', price.toString());
            console.log('paid', paid.toString());
            console.log('wrapper balance', (await link.balanceOf(wrapper.address)).toString());
          ...
          it('Should successfully request a random number and mint an NFT', async () => {
            const { consumer, wrapper, coordinator, link, owner, consumerOwner } = await loadFixture(deployRandomNumberConsumerFixture);
            await fund(link, owner, consumer.address, oneHundredLink);
            // check initial balance of wrapper
            const subscriptionId = 1;
            ...
            const price = calculatePriceNFT(gasLimit);
            const numWords = 1;

            await consumer.connect(consumerOwner).mint({
              gasPrice: oneHundredGwei,
            });
        ...
        describe('error', async () => {
          it('Cannot request randomness without funding wrapper', async () => {
            const { consumer, consumerOwner } = await loadFixture(deployRandomNumberConsumerFixture);
            await expect(
          ...
          it('Cannot request more than maxNumWords ', async () => {
            const { consumer, link, owner, consumerOwner } = await loadFixture(deployRandomNumberConsumerFixture);
            await fund(link, owner, consumer.address, oneHundredLink);
            const gasLimit = 100_000;
          ...
          it('Cannot fulfill if callback gasLimit too low ', async () => {
            const { consumer, link, coordinator, wrapper, owner, consumerOwner } = await loadFixture(deployRandomNumberConsumerFixture);
            await fund(link, owner, consumer.address, oneHundredLink);
