...
describe('DiamondTest', async function () {
  ...
  before(async function () {
    ...
    diamondERC20Facet = await ethers.getContractAt(
      'ERC20Facet',
      diamondAddress
    );
  });
  ...
  it('facets should have the right function selectors -- call to facetFunctionSelectors function', async () => {
    ...
    assert.sameMembers(result, selectors);
    selectors = getSelectors(diamondNftFacet);
  ...
  it('selectors should be associated to facets correctly -- multiple calls to facetAddress function', async () => {
    ...
    assert.equal(
      addresses[3],
      await diamondLoupeFacet.facetAddress('0x42966c68') //burn(uint256)
    );
  });
  ...
  it('add most functions and facets', async () => {
    ...
    const cut = [
      ...
      },
      {
        ...
        action: FacetCutAction.Add,
        functionSelectors: getSelectors(diamondNftFacet),
      },
      {
        facetAddress: addresses[4],
        action: FacetCutAction.Add,
        functionSelectors: getSelectors(diamondERC20Facet),
      },
      {
    ...
    assert.sameMembers(
      ...
      getSelectors(diamondNftFacet)
    );
    assert.sameMembers(
      facets[findAddressPositionInFacets(addresses[4], facets)][1],
      getSelectors(diamondERC20Facet)
    );
    assert.sameMembers(
  ...
  it('should mint an NFT Token1 to user3', async () => {
    // Get 4 different users
    const [user1, user2, user3, user4] = await ethers.getSigners();

    // mint NFT tokenID 1 to user3
    await diamondNftFacet.mint(user3.address, 1);
    expect(await diamondNftFacet.balanceOf(user3.address)).to.equal(1);
    expect(await diamondNftFacet.ownerOf(1)).to.equal(user3.address);
  });

  it('should transfer NFT Token1 from user3 to user4', async () => {
    // Get 4 different users
    const [user1, user2, user3, user4] = await ethers.getSigners();

    // transfer NFT tokenID 1 from user3 to user4
    await diamondNftFacet.connect(user3).transfer(user4.address, 1);
    expect(await diamondNftFacet.balanceOf(user3.address)).to.equal(0);
    expect(await diamondNftFacet.balanceOf(user4.address)).to.equal(1);
    expect(await diamondNftFacet.ownerOf(1)).to.equal(user4.address);
  });

  it("should burn NFT Token1 from user4's account", async () => {
    // Get 4 different users
    const [user1, user2, user3, user4] = await ethers.getSigners();

    // burn NFT tokenID 1 from user4's account
    expect(await diamondNftFacet.balanceOf(user4.address)).to.equal(1);
    await diamondNftFacet.connect(user4).burn(1);
    expect(await diamondNftFacet.balanceOf(user4.address)).to.equal(0);
  });

  it('should mint ERC 20 tokens to user ', async () => {
    // Get 4 different users
    const [user1, user2, user3, user4] = await ethers.getSigners();

    // mint 100 erc20 tokens to user2
    await diamondERC20Facet.erc20mint(user2.address, 100);
  ...
  it('should mint an NFT Token 5 to user2 by transfering 20 ERC20 tokens from user2 to the diamond contract', async () => {
    ...
    // mint NFT tokenID 1 to user2
    await diamondNftFacet.connect(user2).mintWithERC20(5);
    expect(await diamondNftFacet.balanceOf(user2.address)).to.equal(1);
